...
const clues = [
    ...
    { id: 2, question: "在游乐园坐跳楼机时,我说了什么?", answer: "不爱你才和你不一起坐", fragment: "1" },
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function startLoveAnimation() {
        ...
        const internalFlowOffsets = new Float32Array(particleCount * 3);
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            homePositions[idx] = x_o * scale;
            homePositions[idx + 1] = (y_o + yOffset) * scale;
            homePositions[idx + 2] = z_s * scale;
            ...
            internalFlowOffsets[idx] = 0;
            internalFlowOffsets[idx + 1] = 0;
            internalFlowOffsets[idx + 2] = 0;
            flowSpeeds[i] = Math.random() * 0.005 + 0.002;
        }
        ...
        function animate() {
            ...
            if (animationState === 'heart') {
                ...
                } else if (cycleTime > 0.3 && cycleTime < 0.45) {
                    beatScale = 1.0 + secondBeatIntensity * easeInOutQuad((cycleTime - 0.3) / 0.15);
                }
            }
            
            for (let i = 0; i < particleCount; i++) {
                const idx = i * 3;
                let currentTargetX = homePositions[idx];
                let currentTargetY = homePositions[idx+1];
                let currentTargetZ = homePositions[idx+2];

                if (animationState === 'formingFace') {
                    currentTargetX = faceTargetPositions[idx];
                    currentTargetY = faceTargetPositions[idx+1];
                ...
                if (targetPositions) {
                    const targetX = currentTargetX * beatScale;
                    const targetY = currentTargetY * beatScale;
                    const targetZ = currentTargetZ * beatScale;
                    const dx = targetX - positions[idx], dy = targetY - positions[idx+1], dz = targetZ - positions[idx+2];
                    velocities[idx] += dx * forceFactor;
                    velocities[idx+1] += dy * forceFactor;
                    velocities[idx+2] += dz * forceFactor;
                }
        ...
        function handleOrientation(event) { if (event.beta === null || event.gamma === null) return; const newX = event.gamma / 45; const newY = (event.beta - 45) / 20; mouse.x = Math.max(-1, Math.min(1, newX)); mouse.y = Math.max(-1, Math.min(1, -newY)); }
        ...
        window.addEventListener('click', () => {
            ...
            if (faceCoords.length > 0) {
                animationState = 'formingFace'; material.map = glowTexture; material.size = isMobile ? 1.0 : 1.2;
                let shuffledCoords = [...faceCoords].sort(() => 0.5 - Math.random());
                for (let i = 0; i < particleCount; i++) { const idx = i * 3; const target = shuffledCoords[i % shuffledCoords.length]; faceTargetPositions[idx] = target.x; faceTargetPositions[idx+1] = target.y; faceTargetPositions[idx+2] = (Math.random() - 0.5) * 10; }
                stateTimeout = setTimeout(() => { animationState = 'heart'; }, 8000);
            } else {
                animationState = 'exploding'; material.map = heartTexture; material.size = isMobile ? 1.8 : 2.2;
                const explosionStrength = isMobile ? 2.5 : 2.2;
                for(let i=0; i < particleCount; i++){ const idx = i*3; const direction = new THREE.Vector3((Math.random()-0.5), (Math.random()-0.5), (Math.random()-0.5)).normalize().multiplyScalar(explosionStrength * (Math.random()*0.5 + 1.0)); velocities[idx] += direction.x; velocities[idx+1] += direction.y; velocities[idx+2] += direction.z; }
                stateTimeout = setTimeout(() => { animationState = 'heart'; material.map = glowTexture; material.size = isMobile ? 1.0 : 1.2; }, 5000);
            }
        });
        animate();
    }
